...
void CPU::getState(CPUState& state){
    state.A = A;
    state.F = F;
    state.B = B;
    state.C = C;
    ...
    state.SP = SP;
    state.PC = PC;
    state.interrupts = interruptsEnabled;
    state.halted = halted;
    memoryMap.getState(state.memory);
}

void CPU::setState(CPUState const& state){
    A = state.A;
    F = state.F;
    B = state.B;
    C = state.C;
    ...
    SP = state.SP;
    PC = state.PC;
    interruptsEnabled = state.interrupts;
    halted = state.halted;
    memoryMap.setState(state.memory);
}
